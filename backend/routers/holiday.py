...
# Get all holidays
@router.get("/get-all-holidays", response_model=list[HolidayOut])
def list_holidays(db: Session = Depends(get_db)):
    return get_all_holidays(db)

# Add single holiday
@router.post("/add-holiday", response_model=HolidayOut)
def create_holiday(holiday_in: HolidayCreate, db: Session = Depends(get_db)):
    return add_holiday(db, holiday_in)

# Upload CSV
@router.post("/upload-holiday-csv")
...
# Update holiday
@router.put("/update-holiday/{holiday_id}", response_model=HolidayOut)
